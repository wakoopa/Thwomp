...
Requirements:
* giflib
* gstreamer (with plugins) or ffmpeg (with libswscale enabled)
* sdl
* boost
* antigrain (libagg)
* gnash
...
Note: you can skip this step if you're using gstreamer

You can install ffmpeg via homebrew:
  brew install ffmpeg

Or... from source. This is probably a good option becase we will be installing a nightly build from gnash:
  cd /tmp/
  wget http://ffmpeg.org/releases/ffmpeg-0.8.1.tar.gz
  tar -xzf ffmpeg-0.8.1.tar.gz
  cd ffmpeg-0.8.1
  ./configure
  make
  make install
...
+Creating a thumbnail preview for a given SWF:+

Initialize the renderer:
  renderer = Thwomp::Renderer.new("http://www.nu.nl/ad.swf")

  Thwomp::Thumbnail.new(renderer).file
